...
import { Menu, X, Home, Info, Code2, Wrench, ChevronDown } from "lucide-react";
import { Link, NavLink } from "react-router-dom";

const links = {
  home: { label: "Home", to: "/" },
  about: { label: "Team", to: "/team" },
};

const codeItems = [{ label: "Into the Deep", to: "/code/into-the-deep" }];
const buildItems = [{ label: "Into the Deep", to: "/build/into-the-deep" }];
...
const NavBar = () => {
  ...
  const baseLinkClasses =
    "inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-medium text-white hover:bg-lime-900/60";
  ...
  const NavItem = ({ to, icon: Icon, children }) => (
    <NavLink
      ...
      className={({ isActive }) =>
        `${baseLinkClasses}${isActive ? " bg-lime-900/60" : ""}`
      }
    >
          ...
          >
            {items.map((item) => (
              <NavLink
                ...
                to={item.to}
                className={({ isActive }) =>
                  `block rounded-xl px-3 py-2 text-sm text-white hover:bg-lime-900/60${
                    isActive ? " bg-lime-900/60" : ""
                  }`
                }
    ...
    <header className="sticky top-0 z-50 w-full bg-[#224004]">
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 md:px-6">
        ...
        <div className="hidden items-center gap-1 md:flex">
          <NavItem to={links.home.to} icon={Home}>{links.home.label}</NavItem>
          <NavItem to={links.about.to} icon={Info}>{links.about.label}</NavItem>
          <Dropdown
            id="build"
            label="Build"
            items={buildItems}
          ...
          >
            <div className="space-y-1 border-t border-lime-700 p-3">
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-medium text-white hover:bg-lime-900/60${
                    isActive ? " bg-lime-900/60" : ""
                  }`
                }
                ...
                <div className="mt-1 space-y-1 pl-9">
                  {codeItems.map((i) => (
                    <NavLink
                      ...
                      to={i.to}
                      className={({ isActive }) =>
                        `block rounded-lg px-3 py-2 text-sm text-white hover:bg-lime-900/60${
                          isActive ? " bg-lime-900/60" : ""
                        }`
                      }
                      onClick={() => setMobileOpen(false)}
